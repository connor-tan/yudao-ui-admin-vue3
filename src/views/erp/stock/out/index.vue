<template>
  <doc-alert v-if="false" title="【库存】其它入库、其它出库" url="https://doc.iocoder.cn/erp/stock-in-out/" />
...
/** 添加/修改操作 */
const formRef = ref<InstanceType<typeof StockOutForm>>()
...
const handleRowDelete = (row: StockOutVO) => {
  if (row.id == null) {
    return
  }
  handleDelete([row.id])
...
const handleRowStatusChange = (row: StockOutVO, status: number) => {
  if (row.id == null) {
    return
  }
