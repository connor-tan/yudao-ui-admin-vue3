...
import type { StockInItemVO } from '@/api/erp/stock/in'
...
const formData = ref<StockInItemVO[]>([])
...
/** 合计 */
const getSummaries = (param: SummaryMethodProps<StockInItemVO>) => {
  ...
  columns.forEach((column, index) => {
    ...
    if (['count', 'totalPrice'].includes(property)) {
      const sum = getSumValue(data.map((item) => Number(item[property as keyof StockInItemVO])))
      sums[index] =
...
/** 新增按钮操作 */
const createEmptyRow = (): StockInItemVO => ({
  id: undefined,
...
/** 处理仓库变更 */
const onChangeWarehouse = (_warehouseId: number | undefined, row: StockInItemVO) => {
  // 加载库存
  setStockCount(row)
}

/** 处理产品变更 */
const onChangeProduct = (productId: number | undefined, row: StockInItemVO) => {
...
/** 加载库存 */
const setStockCount = async (row: StockInItemVO) => {
