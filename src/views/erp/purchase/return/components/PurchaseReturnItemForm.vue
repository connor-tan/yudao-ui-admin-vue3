...
import type { PurchaseReturnItemVO } from '@/api/erp/purchase/return'
...
const formData = ref<PurchaseReturnItemVO[]>([])
...
/** 合计 */
const getSummaries = (param: SummaryMethodProps<PurchaseReturnItemVO>) => {
  ...
  columns.forEach((column, index: number) => {
    ...
    if (['count', 'totalProductPrice', 'taxPrice', 'totalPrice'].includes(property)) {
      const sum = getSumValue(
        data.map((item) => Number(item[property as keyof PurchaseReturnItemVO]))
      )
      sums[index] =
...
/** 加载库存 */
const setStockCount = async (row: PurchaseReturnItemVO) => {
  if (!row.productId) {
    return
  }
...
/** 处理仓库变更 */
const onChangeWarehouse = (_warehouseId: number | undefined, row: PurchaseReturnItemVO) => {
  setStockCount(row)
}
