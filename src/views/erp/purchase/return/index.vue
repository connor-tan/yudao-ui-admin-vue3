<template>
  <doc-alert v-if="false" title="【采购】采购订单、入库、退货" url="https://doc.iocoder.cn/erp/purchase/" />
...
/** 添加/修改操作 */
const formRef = ref<InstanceType<typeof PurchaseReturnForm>>()
...
const handleRowDelete = (row: PurchaseReturnVO) => {
  if (row.id == null) {
    return
  }
  handleDelete([row.id])
...
const handleRowStatusChange = (row: PurchaseReturnVO, status: number) => {
  if (row.id == null) {
    return
  }
