...
import type { SaleOutItemVO } from '@/api/erp/sale/out'
...
const formData = ref<SaleOutItemVO[]>([])
...
/** 合计 */
const getSummaries = (param: SummaryMethodProps<SaleOutItemVO>) => {
  ...
  columns.forEach((column, index: number) => {
    ...
    if (['count', 'totalProductPrice', 'taxPrice', 'totalPrice'].includes(property)) {
      const sum = getSumValue(data.map((item) => Number(item[property as keyof SaleOutItemVO])))
      sums[index] =
...
/** 加载库存 */
const setStockCount = async (row: SaleOutItemVO) => {
  if (!row.productId) {
    return
  }
...
/** 处理仓库变更 */
const onChangeWarehouse = (_warehouseId: number | undefined, row: SaleOutItemVO) => {
  setStockCount(row)
}
