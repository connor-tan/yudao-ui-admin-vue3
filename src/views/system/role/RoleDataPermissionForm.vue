...
const deptOptions = ref<(DeptApi.DeptVO & { children?: DeptApi.DeptVO[] })[]>([]) // 部门树形结构
const deptExpand = ref(true) // 展开/折叠
const treeRef = ref<TreeInstance>() // 菜单树组件 Ref
const treeNodeAll = ref(false) // 全选/全不选
const checkStrictly = ref(true) // 是否严格模式，即父子不关联

const collectDeptIds = (depts: (DeptApi.DeptVO & { children?: DeptApi.DeptVO[] })[]): number[] => {
  return depts.flatMap((dept) => [dept.id, ...collectDeptIds(dept.children ?? [])])
}
...
const open = async (row: RoleApi.RoleVO) => {
  ...
  deptOptions.value = handleTree(await DeptApi.getSimpleDeptList()) as (
    DeptApi.DeptVO & { children?: DeptApi.DeptVO[] }
  )[]
  // 设置数据
  formData.id = row.id
...
const handleCheckedTreeNodeAll = () => {
  treeRef.value?.setCheckedKeys(treeNodeAll.value ? collectDeptIds(deptOptions.value) : [])
}
