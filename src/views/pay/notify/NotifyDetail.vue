  ...
  <Dialog v-model="dialogVisible" title="通知详情" width="50%">
    <el-descriptions :column="2">
      <el-descriptions-item label="通知状态" :span="2">
        <dict-tag :type="DICT_TYPE.PAY_NOTIFY_STATUS" :value="detailData.status ?? 0" />
      ...
      <el-descriptions-item label="通知类型">
        <dict-tag :type="DICT_TYPE.PAY_NOTIFY_TYPE" :value="detailData.type ?? 0" />
      </el-descriptions-item>
...
const open = async (id: number) => {
  ...
  try {
    ...
    if (!detailData.value.logs) {
      detailData.value.logs = []
    }
  } finally {
