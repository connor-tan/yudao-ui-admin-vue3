...
type ExpressTemplateChargeVO = DeliveryExpressTemplateApi.ExpressTemplateChargeVO
type ExpressTemplateFreeVO = DeliveryExpressTemplateApi.ExpressTemplateFreeVO
type DeliveryExpressTemplateFormData = DeliveryExpressTemplateApi.DeliveryExpressTemplateVO
type ColumnTitle = {
  startCountTitle: string
  extraCountTitle: string
  freeCountTitle: string
}
...
const createChargeItem = (): ExpressTemplateChargeVO => ({
  areaIds: [],
  startCount: 1,
  startPrice: 1,
  extraCount: 1,
...
const createFreeItem = (): ExpressTemplateFreeVO => ({
  areaIds: [],
  freeCount: 1,
  freePrice: 1
})
const createFormData = (): DeliveryExpressTemplateFormData => ({
  id: undefined,
  name: '',
  chargeMode: 1,
  sort: 0,
...
const formData = ref<DeliveryExpressTemplateFormData>(createFormData())
const columnTitleMap = new Map<number, ColumnTitle>()
const columnTitle = ref<ColumnTitle>({
  startCountTitle: '首件',
  extraCountTitle: '续件',
  freeCountTitle: '包邮件数'
})
...
const areaTree = ref<AreaApi.AreaNodeVO[]>([])
...
const open = async (type: string, id?: number) => {
  ...
  try {
    ...
    if (id) {
      ...
      formData.value.charges = formData.value.charges ?? formData.value.templateCharge ?? []
      formData.value.frees = formData.value.frees ?? formData.value.templateFree ?? []
      columnTitle.value = columnTitleMap.get(formData.value.chargeMode) ?? columnTitle.value
...
const resetForm = () => {
  ...
  columnTitle.value = columnTitleMap.get(1) ?? columnTitle.value
  formRef.value?.resetFields()
}

/** 配送计费方法改变 */
const changeChargeMode = (chargeMode: number) => {
  columnTitle.value = columnTitleMap.get(chargeMode) ?? columnTitle.value
}
...
const addChargeArea = () => {
  ...
  data.charges.push(createChargeItem())
}
...
const addFreeArea = () => {
  const data = formData.value
  data.frees.push(createFreeItem())
}
