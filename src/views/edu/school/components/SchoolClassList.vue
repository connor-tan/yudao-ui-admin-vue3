...
const list = ref<SchoolClass[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await SchoolApi.getSchoolClassPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await SchoolApi.deleteSchoolClass(id)
...
const handleDeleteBatch = async () => {
  try {
    // 删除的二次确认
    await message.delConfirm()
    await SchoolApi.deleteSchoolClassList(checkedIds.value)
    checkedIds.value = []
    message.success(t('common.delSuccess'))
    await getList()
  } catch {}
...
const handleRowCheckboxChange = (records: SchoolClass[]) => {
